...
async function runAutoDumpSnapshot() {
    try {
        console.log('[AutoDump] Đang tạo bản sao lưu snapshot tự động...');
        const dumpFilePath = path.join(__dirname, '../database_dump.sql');
        let sql = '';

        // Header SQL
        sql += `--\n-- PostgreSQL database dump (Auto-Generated via Web Change)\n--\n\n`;
        ...
        console.log(`[AutoDump] Đã lưu tự động snapshot thành công vào file: database_dump.sql`);
    } catch (err) {
        console.error('[AutoDump] Lỗi trong quá trình tự động sao lưu:', err.message);
    }
}
...
function triggerAutoDump() {
    ...
    timeoutId = setTimeout(() => {
        ...
        // Chạy bất đồng bộ
        runAutoDumpSnapshot();
    }, 15000); // Debounce 15 giây
}

function autoDumpMiddleware(req, res, next) {
    // Chỉ kích hoạt tự động dump cho các request thay đổi dữ liệu (POST, PUT, DELETE)
    // Loại trừ các API liên quan đến Authentication (Đăng nhập, đăng ký)
    if (['POST', 'PUT', 'DELETE'].includes(req.method) && !req.originalUrl.startsWith('/api/auth')) {
        res.on('finish', () => {
            if (res.statusCode >= 200 && res.statusCode < 300) {
                console.log(`[AutoDump] Phát hiện thay đổi dữ liệu từ API: ${req.method} ${req.originalUrl}`);
                triggerAutoDump();
            }
        });
    }
